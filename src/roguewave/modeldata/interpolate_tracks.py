...
def interpolate_points_data_arrray(data_array: DataArray,
    ...
    :param cyclic_coordinates: Dictonary that contains as keys the coordinate
        names of cyclic coordinates, and as value the cyclic length of the
        coordinate.

    :param cyclic_length_data: Cyclic length of the data. If data is not cyclic
    ...
    if cyclic_length_data is None:
        ...
        for value, weight in _recursive_loop_over_coordinates(
            ...
            weights_sum += weight
            total  += value*weight
        output[:] = numpy.where(weights_sum>0,total / weights_sum,numpy.nan)
    else:
        ...
        cosine = numpy.where(weights_sum>0,cosine/weights_sum, numpy.nan)
        sine = numpy.where(weights_sum>0,sine/weights_sum,numpy.nan)
