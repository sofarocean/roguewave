...
from roguewave.wavephysics.balance.wam_tail_stress import log_mu_log_x
...
def tail_stress_parametrization_jb23(
    ...
    if wind_forcing_type == "u10":
        friction_velocity = (
                wind_forcing * parameters["vonkarman_constant"] / log(parameters["elevation"] / roughness_length)
        )
    ...
    angular_frequency = dispersion(wavenumbers, parameters["gravitational_acceleration"], parameters["surface_tension"])

    stress = 0.0
    for wavenumber_index in range(1, wavenumbers.shape[0]):
        wavenumber_step = wavenumbers[wavenumber_index] - wavenumbers[wavenumber_index - 1]

        stress += (
                          angular_frequency[wavenumber_index - 1]
                          * windinput[wavenumber_index - 1] / 2
                          + angular_frequency[wavenumber_index]
                          * windinput[wavenumber_index] / 2
                  ) * wavenumber_step
...
def wind_stress_tail(
    ...
    angular_frequency = dispersion(
        wavenumbers, parameters["gravitational_acceleration"], parameters["surface_tension"])

    stress = empty(wavenumbers.shape[0])
    for wavenumber_index in range(0, wavenumbers.shape[0]):
        stress[wavenumber_index] = angular_frequency[wavenumber_index] * windinput[wavenumber_index]
...
def log_bounds_wavenumber(
    ...
    :return:
    """
    args = (roughness_length,
            friction_velocity,
            parameters
            )

    # find the right root
    log_upper_bound = numba_newton_raphson(miles_mu_cutoff, log(1 / roughness_length), args, (-inf, log(1 / roughness_length)),
                                           verbose=False, name='log bound wavenumber 1')

    args_old = (
        roughness_length * parameters['gravitational_acceleration'] / friction_velocity ** 2,
    ...
    # loglog_mu = 0
    x0 = numba_newton_raphson(log_mu_log_x, log(0.01), args_old, (-5, log_upper_bound), verbose=False,
                              name='log bound wavenumber 2')
    guess = x0

    while True:
        # print(guess, log_upper_bound)
        log_lower_bound = numba_newton_raphson(miles_mu, guess, args, (-10, log_upper_bound), verbose=False,
...
def miles_mu(log_wavenumber,
    ...
    return log_wavenumber + log(roughness_length) + vonkarman_constant / (
...
def wind_input_tail(
    ...
    wave_speed = celerity(wavenumber, parameters["gravitational_acceleration"], 0. ) #parameters["surface_tension"])
    angular_frequency = dispersion(wavenumber, parameters["gravitational_acceleration"], parameters["surface_tension"])
    k3w = three_wave_starting_wavenumber(parameters["gravitational_acceleration"], parameters["surface_tension"],friction_velocity)
    ...
    for wavenumber_index in range(0, number_of_wavenumbers):
        ...
        linear_growth_parameter = (
                ...
                * growth_parameter_betamax / vonkarman_constant ** 2
                * epsilon
                * friction_velocity ** 2
        ...
        windinput[wavenumber_index] = integration_width * growth_parameter * tail_spectrum[wavenumber_index]
...
def saturation_spectrum_parametrization(
    ...
    three_wave_start = three_wave_starting_wavenumber(gravitational_acceleration, surface_tension, friction_velocity)
    ...
    strength_three_wave_interactions = 3 * pi / 16 * (tanh(2 * (sqrt(wavenumbers / three_wave_start) - 1)) + 1)
    ...
    for wavenumber_index in range(number_of_wavenumbers):
        if wavenumbers[wavenumber_index] > three_wave_start:
            ...
            spectrum[wavenumber_index] = scaling_constant * y * sqrt(1 + 3 * y ** 2) / (
                    (1 + y ** 2) * (y + y ** 3) ** (1 / 4))
...
@njit(cache=True)
def three_wave_starting_wavenumber(gravitational_acceleration, surface_tension, friction_velocity):
    """
    ...
    :return:
    """
    return sqrt(gravitational_acceleration / surface_tension) * 1 / (1.48 + 2.05 * friction_velocity)
...
def dispersion(wavenumber, gravitational_acceleration, surface_tension):
    return sqrt(gravitational_acceleration * wavenumber + surface_tension * wavenumber ** 3)
