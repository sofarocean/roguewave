...
from roguewave.wavephysics.roughness import charnock_roughness_length_from_u10, drag_coefficient_charnock
from roguewave.wavespectra.parametric import create_parametric_frequency_direction_spectrum
from roguewave import FrequencyDirectionSpectrum, concatenate_spectra, load
...
def m0_from_wave_age( wave_age, peak_frequency):
    ...
    g = 9.81
    gamma = 3.3
    return alpha * g**2 / (2*np.pi) **4 / peak_frequency**4 * (0.06533*gamma**0.8015+0.13467)
...
def spectrum_from_u10_and_wave_age_guess( U10, wave_age ) -> FrequencyDirectionSpectrum:
    ...
    hs = 4 * np.sqrt(m0)
    return create_parametric_frequency_direction_spectrum(resolved_frequencies,peak_frequency,hs)
...
def gen_saturation_spectrum(u10, wave_age=25):
    ...
    energy_at_starting_wavenumber = spec1d.group_velocity.values[-1] * spec1d.variance_density.values[-1] /2/np.pi
    starting_wavenumber = spec1d.wavenumber.values[-1]

    wavenumbers = jb23.wavenumber_grid(starting_wavenumber,
                                       roughness_length,
                                       friction_velocity,
                                       parameters)

    return wavenumbers, jb23.saturation_spectrum_parametrization(
        wavenumbers, energy_at_starting_wavenumber, starting_wavenumber, friction_velocity, parameters
    )
...
def t_wavenumber_grid():
    ...
    three_wave_limit = jb23.three_wave_starting_wavenumber(friction_velocity, parameters)
    ...
    def gen_grid(start, isempty, has_eq, has_cons, has_3w):
        ...
        assert has_cons == (np.any((grid < three_wave_limit) & (grid >= eq_range_limit)) and np.all(grid > 0))
...
def t_log_bounds():
    ...
    for u10val in u10_vec:
        ...
        bounds = jb23.log_bounds_wavenumber(roughness_length, friction_velocity, parameters)

        if np.isfinite(bounds[1]):
            lower_bound_miles = jb23.miles_mu_cutoff(bounds[0], roughness_length, friction_velocity, parameters)
            upper_bound_miles = jb23.miles_mu_cutoff(bounds[1], roughness_length, friction_velocity, parameters)
...
def t_sat_spec():
    ...
    wind_input = jb23.wind_input_tail(
        k, roughness, fric_vel, en_spec,parameters
    )
    stress = jb23.wind_stress_tail( k, roughness,fric_vel,en_spec,parameters )

    plt.plot( k, sat_spec / sat_spec[0])
    plt.xscale('log')
    plt.figure()
    # plt.plot( k, wind_input)
    # plt.xscale('log')
    #
    # plt.figure()
    # plt.plot( k, stress)
...
def t_stress():
    ...
    spectrum = load('/Users/pietersmit/PycharmProjects/train_wind_algo/scratch/spec.zip')

    # def _calc_stress(u10,roughness_length):
    #     spectrum = spectrum_from_u10_and_wave_age_guess(U10=u10, wave_age=25)
    #     wind = (u10,0,'u10')
    #
    #     stress = jb23.tail_stress_parametrization_jb23(
    #         spectrum.variance_density.values,
    #         wind,
    #         np.inf,
    #         roughness_length,
    #         _input.spectral_grid(spectrum),
    #         parameters,
    #     )


    roughness = 10**np.linspace( -6,-1,100 )
